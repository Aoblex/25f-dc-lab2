...
2.  **候选拼车对构造：**找出在相近时间、相邻地点出发的乘客组合。

3.  **模型训练：**使用 Spark MLlib
    训练一个二分类模型，用于判断是否可以拼车。
...
1.  硬件配置：集群节点数量、每节点 CPU 核心数、内存容量及 Spark
    的核心配置（例如 executor.instances、executor.memory、driver.memory
    等）。

2.  数据集规模：数据来源与时间范围（例如 2024 年 1--3
    月），文件格式、总大小以及分区策略。
